...
bool ADXL345::init(int address) 
{
	...
	if( (i2c_fd = wiringPiI2CSetup( address )) < 0)
	{
		fprintf (stderr, "Unable to open ADXL345 accelerometer: %s\n", strerror (errno)) ;
		return false;
	}
	else
	{
...
void ADXL345::writeTo(U8 address, U8 val) 
{
  wiringPiI2CWriteReg8( i2c_fd, address, val );
}
...
void ADXL345::getRangeSetting(U8* rangeSetting) 
{
  U8 _b;
...
void ADXL345::setAxisOffset(int x, int y, int z) {
  writeTo(ADXL345_OFSX, U8 (x));  
  writeTo(ADXL345_OFSY, U8 (y));  
  writeTo(ADXL345_OFSZ, U8 (z));  
}
...
void ADXL345::setRate(float rate)
{
  ...
  {
    readFrom(ADXL345_BW_RATE, 1, &_b);
    //_s = (U8) (r + 6) | (_b & B11110000);
    _s = (U8) (r + 6) | (_b & 0xF0);
    writeTo(ADXL345_BW_RATE, _s);
  }
}

void ADXL345::set_bw(U8 bw_code){
...
U8 ADXL345::get_bw_code(){
  U8 bw_code;
  readFrom(ADXL345_BW_RATE, 1, &bw_code);
  return bw_code;
}

U8 ADXL345::getInterruptSource() {
  U8 _b;
  readFrom(ADXL345_INT_SOURCE, 1, &_b);
  return _b;
}

bool ADXL345::getInterruptSource(U8 interruptBit) {
  return getRegisterBit(ADXL345_INT_SOURCE,interruptBit);
}

bool ADXL345::getInterruptMapping(U8 interruptBit) {
...
void ADXL345::setInterruptMapping(U8 interruptBit, bool interruptPin) {
  setRegisterBit(ADXL345_INT_MAP, interruptBit, interruptPin);
}

bool ADXL345::isInterruptEnabled(U8 interruptBit) {
  return getRegisterBit(ADXL345_INT_ENABLE,interruptBit);
}

void ADXL345::setInterrupt(U8 interruptBit, bool state) {
  setRegisterBit(ADXL345_INT_ENABLE, interruptBit, state);
}

void ADXL345::setRegisterBit(U8 regAdress, int bitPos, bool state) {
  U8 _b;
...
bool ADXL345::getRegisterBit(U8 regAdress, int bitPos) {
  U8 _b;
  readFrom(regAdress, 1, &_b);
  return ((_b >> bitPos) & 1);
}
