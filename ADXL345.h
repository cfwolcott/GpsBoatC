...
public:
	...
	void set_bw(U8 bw_code);
	U8 get_bw_code();  

	U8 getInterruptSource();
	bool getInterruptSource(U8 interruptBit);
	bool getInterruptMapping(U8 interruptBit);
	void setInterruptMapping(U8 interruptBit, bool interruptPin);
	bool isInterruptEnabled(U8 interruptBit);
	void setInterrupt(U8 interruptBit, bool state);

	void getRangeSetting(U8* rangeSetting);
	...
	void writeTo(U8 address, U8 val);
